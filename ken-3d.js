...
function setup(host,mini=false){
  const scene=new T.Scene(),camera=new T.PerspectiveCamera(mini?28:34,1,.1,100);
  camera.position.set(0,mini?1.8:1.65,mini?5.5:5.0);
  ...
  if(!mini&&T.OrbitControls){
    ...
    controls.enableDamping=true;controls.dampingFactor=.07;controls.target.set(0,1.42,0);
    controls.minDistance=2.6;controls.maxDistance=7.2;controls.enablePan=true;controls.autoRotate=true;controls.autoRotateSpeed=.45;
...
function init(){
  const lh=document.getElementById("ken-large-3d"),mh=document.getElementById("ken-mini-3d");
  if(lh)large=setup(lh,false);if(mh)mini=setup(mh,true);
...
function animate(model,time,miniMode=false){
  ...
  if(miniMode)model.rotation.y=.20*Math.sin(time*.0007);
