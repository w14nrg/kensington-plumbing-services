...
(function(){
  ...
  root.innerHTML=`
    <header class="ken-chat-head">
      <div class="ken-mini-avatar"><img src="/ken-avatar.jpg" alt="Ken"></div>
  ...
  function add(text,role="bot",persist=true){
    ...
    row.innerHTML=role==="bot"?`<div class="ken-bot-avatar"><img src="/ken-avatar.jpg" alt=""></div><div class="ken-bubble">${esc(text).replace(/\n/g,"<br>")}</div>`:`<div class="ken-bubble">${esc(text).replace(/\n/g,"<br>")}</div>`;
  ...
  function typing(on){root.querySelector("[data-typing]")?.remove();if(on){const t=document.createElement("div");t.className="ken-typing";t.dataset.typing="1";t.textContent="Ken is thinking…";messages.appendChild(t)}}
  ...
  async function fallback(message){
    ...
    if(f.step===0){const j=infer(message);f.job=j;f.step=1;save();return{reply:j.q,progress:38}}
    if(f.step===1){f.detail=message;f.step=2;save();return{reply:"Thanks. Is the area easy to get to, tight/awkward, or hidden behind tiles, boxing or flooring?",progress:58,quickReplies:["Easy to get to","Tight or awkward","Hidden behind tiles or boxing"]}}
    if(f.step===2){f.access=message;f.step=3;save();return{reply:"What’s the postcode for the job?",progress:75}}
    if(f.step===3){
      f.postcode=postcode(message)||message;f.step=4;save();
      let min=f.job.min,max=f.job.max;if(/tight|awkward/i.test(f.access)){min+=35;max+=90}if(/hidden|tile|boxing|floor/i.test(f.access)){min+=80;max+=220}
  ...
  async function send(text){
    ...
    let data=null;
    try{data=await api("/api/ken",{method:"POST",body:JSON.stringify({sessionId:state.sessionId,message,history:state.conversation.slice(0,-1),state:state.serverState,page:location.pathname})})}
    catch{data=await fallback(message)}
    typing(false);if(data.state)state.serverState=data.state;if(data.reply)add(data.reply,"bot");if(data.quickReplies)setQuick(data.quickReplies);progress(data.progress||40);if(data.estimate)showEstimate(data.estimate);save()
  }

  function showEstimate(e){
    html(`<div class="ken-estimate"><div class="ken-estimate-label">YOUR ESTIMATED REPAIR</div><h3>${esc(e.jobName)}</h3><div class="ken-price">£${esc(e.min)}–£${esc(e.max)}</div><p>${esc(e.summary||"Based on the details supplied.")}</p><div class="ken-pay-note"><strong>£75 to book your plumber.</strong><br>The £75 covers attendance and diagnosis and is deducted from the final repair price when we carry out the work.</div><button class="ken-action-btn" type="button" data-book-preview>See available appointments</button></div>`);
  ...
  function boot(){
    ...
    else add("Hi, I’m Ken. Tell me what’s gone wrong in your own words and I’ll help work out the likely problem, give you an estimated price range and help you book a plumber.","bot");
    setQuick(["I have a leak","My toilet has a problem","I have a tap or shower problem","I have a radiator or heating problem","I have a blocked sink or drain"]);progress(18)
  }
