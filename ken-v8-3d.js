...
function labelPlane(text,bg,fg,w=256,h=128,fs=64){
  const c=document.createElement("canvas");c.width=w;c.height=h;
  const x=c.getContext("2d");
  x.fillStyle=bg;x.fillRect(0,0,w,h);
  x.strokeStyle="rgba(255,255,255,.22)";x.lineWidth=4;x.strokeRect(2,2,w-4,h-4);
  x.fillStyle=fg;x.font=`700 ${fs}px Arial`;x.textAlign="center";x.textBaseline="middle";
  x.fillText(text,w/2,h/2);
  const tex=new T.CanvasTexture(c);tex.anisotropy=4;
  const mat=new T.MeshStandardMaterial({map:tex,transparent:false,roughness:.55,metalness:.04});
  return new T.Mesh(new T.PlaneGeometry(w/260,h/260),mat);
}
...
function makeKen(){
  ...
  // KPS shirt text and Ken name badge
  const kpsLabel=labelPlane("KPS", "#0b2e43", "#ffffff", 240, 110, 68);
  kpsLabel.position.set(-.13,1.78,.415);
  kpsLabel.scale.set(.34,.16,1);
  root.add(kpsLabel);
  const kenBadge=labelPlane("KEN", "#f5f0e7", "#0b2930", 250, 120, 62);
  kenBadge.position.set(.19,1.73,.417);
  kenBadge.rotation.z=-.08;
  kenBadge.scale.set(.24,.12,1);
  root.add(kenBadge);
  const badgeClip=box(.035,.06,.02,0x98a7a9);badgeClip.position.set(.10,1.83,.412);root.add(badgeClip);
  ...
  // short, normal black hair
  const hairCap=sph(.295,0x121212,1.02,.34,.94);
  hairCap.position.set(0,2.68,-.02);
  root.add(hairCap);
  const frontHair=sph(.19,0x121212,1.10,.24,.76);
  frontHair.position.set(0,2.70,.13);
  root.add(frontHair);
  ...
  // smile + teeth
  const mouth=mesh(new T.TorusGeometry(.090,.011,10,28,Math.PI),m(C.dark,.28,.01));
  mouth.position.set(0,2.345,.298);mouth.rotation.z=Math.PI;mouth.scale.y=.70;root.add(mouth);
  const teeth=box(.11,.020,.012,C.teeth);teeth.position.set(0,2.366,.307);root.add(teeth);
...
function setup(host,mini=false){
  const scene=new T.Scene(),camera=new T.PerspectiveCamera(mini?27:34,1,.1,100);
  if(mini){
    camera.position.set(0,2.35,4.0);
    camera.lookAt(0,2.28,0);
  }else{
    camera.position.set(0,1.58,5.45);
  }
  ...
  const ken=makeKen();
  if(mini){
    ken.scale.multiplyScalar(.98);
    ken.position.y=-.02;
  }
  scene.add(ken);
  if(!mini){scene.add(makeStage())}

  let controls=null,lastInteract=0;
  if(!mini&&T.OrbitControls){
    controls=new T.OrbitControls(camera,renderer.domElement);
    controls.enableDamping=true;controls.dampingFactor=.07;controls.target.set(0,1.48,0);
    controls.minDistance=3.6;controls.maxDistance=7.4;controls.enablePan=true;controls.autoRotate=true;controls.autoRotateSpeed=.40;
...
function animate(model,time,miniMode=false){
  ...
  if(miniMode)model.rotation.y=.035*Math.sin(time*.0007);
