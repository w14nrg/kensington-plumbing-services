...
root.innerHTML=`<header class="ken-chat-head"><div class="ken-mini-avatar"><div id="ken-mini-3d" class="ken-mini-3d-host" style="width:100%;height:100%"></div></div><div class="ken-chat-title"><strong>Ken</strong><span>Plumbing assistant · Estimates · Booking</span></div><div class="online-pill">Online</div></header><div class="ken-progress"><span></span></div><div class="ken-confidence-line"><span>Estimate confidence</span><strong>Tell Ken what’s happening</strong></div><div id="ken-live-estimate" class="ken-live-estimate" hidden></div><div class="ken-messages" aria-live="polite"></div><div class="ken-quick"></div><div class="ken-inputbar"><input type="text" maxlength="700" placeholder="Describe your plumbing problem…"><button class="ken-send" type="button">➜</button></div><div class="ken-footnote">The estimate adjusts as Ken learns more. * Estimates are not fixed quotations. <button type="button" data-ken-disclaimer>How estimates work</button></div>`;
...
function boot(){if(state.conversation.length)state.conversation.forEach(m=>add(m.content,m.role==="user"?"user":"bot",false));else add("Hi, I’m Ken. Tell me what’s gone wrong in your own words. I’ll talk it through with you and keep refining the estimated price as I learn more.","bot");if(state.estimate)renderEstimate(state.estimate,false);setQuick(["I have a leak","My toilet has a problem","I have a tap or shower problem","I have a radiator or heating problem","I have a blocked sink or drain"]);if(!state.estimate)progress(state.serverState?.confidenceScore||15)}

const kenDisclaimer=document.createElement("div");
kenDisclaimer.className="ken-disclaimer-modal";
kenDisclaimer.hidden=true;
kenDisclaimer.innerHTML=`<div class="ken-disclaimer-card" role="dialog" aria-modal="true" aria-label="About Ken's estimates">
...
document.body.appendChild(kenDisclaimer);
kenDisclaimer.addEventListener("click",e=>{
  if(e.target===kenDisclaimer||e.target.closest(".ken-disclaimer-close"))kenDisclaimer.hidden=true;
});
document.addEventListener("click",e=>{
  if(e.target.closest("[data-ken-disclaimer]"))kenDisclaimer.hidden=false;
});

document.dispatchEvent(new CustomEvent("ken-chat-ready"));root.querySelector(".ken-send").onclick=()=>{const t=input.value.trim();if(t){input.value="";send(t)}};input.addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();root.querySelector(".ken-send").click()}});document.querySelectorAll("[data-ken-prompt]").forEach(b=>b.addEventListener("click",()=>{send(b.dataset.kenPrompt);root.scrollIntoView({behavior:"smooth",block:"center"})}));boot();
})();